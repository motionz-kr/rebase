...
export type Driver = 'mysql' | 'postgres' | 'sqlite' | 'sqlserver';
...
export function buildModifyColumn(driver: Driver, table: string, before: ColumnSpec, after: ColumnSpec): string[] {
  ...
  if (driver === 'sqlserver') {
    // T-SQL ALTER COLUMN handles type + nullability in one statement; a DEFAULT
    // is a separate constraint, applied best-effort when it changed.
    const out: string[] = [];
    if (typeChanged || nullChanged) {
      out.push(`ALTER TABLE ${t} ALTER COLUMN ${c} ${after.type.trim()}${after.nullable ? ' NULL' : ' NOT NULL'}`);
    }
    if (defChanged) {
      const def = after.defaultValue?.trim() ?? '';
      if (def !== '') out.push(`ALTER TABLE ${t} ADD DEFAULT ${def} FOR ${c}`);
    }
    return out;
  }
...
export function buildCreateTable(driver: Driver, table: string, columns: CreateColumnSpec[]): string[] {
  ...
  const lines = cols.map((c) => {
    let s = `${q(c.name)} ${c.type.trim()}`;
    if (c.autoIncrement && driver === 'sqlserver') {
      // SQL Server IDENTITY columns are implicitly NOT NULL; the table-level PK
      // covers nullability, so don't emit a separate NOT NULL.
      s += ' IDENTITY(1,1)';
    } else {
