...
func (r *SQLiteMCPActivityRepository) Append(ctx context.Context, event *domain.MCPActivityEvent) error {
	_, err := r.db.ExecContext(ctx, `
		...
		(id, workspace_id, profile_id, server_id, direction, event, tool, status, error_message, duration_ms, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, event.ID, event.WorkspaceID, event.ProfileID, event.ServerID, event.Direction, event.Event, event.Tool, event.Status, event.Error, event.DurationMs, event.CreatedAt)
	return err
}
...
func (r *SQLiteMCPActivityRepository) List(ctx context.Context, filter domain.MCPActivityFilter) ([]domain.MCPActivityEvent, error) {
	...
	query.WriteString(`SELECT id, workspace_id, profile_id, server_id, direction, event, tool, status, error_message, duration_ms, created_at FROM mcp_activity_events WHERE workspace_id = ?`)
	...
	for rows.Next() {
		...
		if err := rows.Scan(&event.ID, &event.WorkspaceID, &event.ProfileID, &event.ServerID, &event.Direction, &event.Event, &event.Tool, &event.Status, &event.Error, &event.DurationMs, &event.CreatedAt); err != nil {
			return nil, err
		}
