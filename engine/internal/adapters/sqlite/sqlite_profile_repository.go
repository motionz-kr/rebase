...
func (r *SQLiteProfileRepository) List(ctx context.Context) ([]domain.ConnectionProfile, error) {
	...
	for rows.Next() {
		var p domain.ConnectionProfile
		err := rows.Scan(&p.ID, &p.Name, &p.Driver, &p.Host, &p.Port, &p.Database, &p.Username, &p.SecretRef, &p.TLSMode, &p.McpEnabled, &p.McpDataExposure, &p.ReadOnly, &p.ConnectionURI, &p.SafeMode, &p.TenantColumns, &p.DomainBindings, &p.DomainGlossary, &p.DomainNotes, &p.CreatedAt, &p.UpdatedAt)
		if err != nil {
			return nil, err
		}
		list = append(list, p)
