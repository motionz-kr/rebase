...
func (r *SQLiteProfileRepository) Create(ctx context.Context, p *domain.ConnectionProfile) error {
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO connection_profiles (id, name, driver, host, port, database, username, secret_ref, tls_mode, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, p.ID, p.Name, p.Driver, p.Host, p.Port, p.Database, p.Username, p.SecretRef, p.TLSMode, p.CreatedAt, p.UpdatedAt)
	return err
}

func (r *SQLiteProfileRepository) GetByID(ctx context.Context, id string) (*domain.ConnectionProfile, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT id, name, driver, host, port, database, username, secret_ref, tls_mode, created_at, updated_at
		FROM connection_profiles WHERE id = ?
	`, id)

	var p domain.ConnectionProfile
	err := row.Scan(&p.ID, &p.Name, &p.Driver, &p.Host, &p.Port, &p.Database, &p.Username, &p.SecretRef, &p.TLSMode, &p.CreatedAt, &p.UpdatedAt)
...
func (r *SQLiteProfileRepository) List(ctx context.Context) ([]domain.ConnectionProfile, error) {
	...
	for rows.Next() {
		var p domain.ConnectionProfile
		err := rows.Scan(&p.ID, &p.Name, &p.Driver, &p.Host, &p.Port, &p.Database, &p.Username, &p.SecretRef, &p.TLSMode, &p.CreatedAt, &p.UpdatedAt)
		if err != nil {
			return nil, err
		}
		list = append(list, p)
...
func (r *SQLiteProfileRepository) Update(ctx context.Context, p *domain.ConnectionProfile) error {
	...
	res, err := r.db.ExecContext(ctx, `
		...
		SET name = ?, driver = ?, host = ?, port = ?, database = ?, username = ?, secret_ref = ?, tls_mode = ?, updated_at = ?
		WHERE id = ?
	`, p.Name, p.Driver, p.Host, p.Port, p.Database, p.Username, p.SecretRef, p.TLSMode, p.UpdatedAt, p.ID)
	if err != nil {
		return err
	}
